...
const ShelvingFloor = ({ navigation }) => {
  ...
  const [shelvingValuestart, setShelvingValueStart] = useState("0");
  const [shelvingValueEnd, setShelvingValueEnd] = useState("0");
  const [floorValuestart, setFloorValueStart] = useState("0");
  const [floorValueEnd, setFloorValueEnd] = useState("0");
  ...
  const simpanShelvingFloor = () => {
    if (
      shelvingValuestart == "0" ||
      shelvingValueEnd == "0" ||
      floorValuestart == "0" ||
      floorValueEnd == "0"
    ) {
      Alert.alert("Warning", "Data shelving dan floor tidak boleh kosong");
      return;
    }
    if (image == null) {
      Alert.alert("Warning", "Anda wajib mengambil foto terlebih dahulu");
      return;
    }
                  ...
                  if (pathSave !== "undefined") {
                    const requestOptions = {
                      ...
                      body: JSON.stringify({
                        ...
                        path: pathSave,
                        nik: lokasi.nik,
                        shelvingstart: Number.parseFloat(
                          shelvingValuestart.replace(/,/g, "")
                        ),
                        shelvingend: Number.parseFloat(
                          shelvingValueEnd.replace(/,/g, "")
                        ),
                        floorstart: Number.parseFloat(
                          floorValuestart.replace(/,/g, "")
                        ),
                        floorend: Number.parseFloat(
                          floorValueEnd.replace(/,/g, "")
                        ),
                        lokasiname: lokasi.customer,
                      }),
                    };
                    // console.log("kirim data foto :", requestOptions);
                    const url = constants.loginServer + "/insertshelvingfloor";
                    fetch(url, requestOptions).then(async (response) => {
                      ...
                      if (!response.ok) {
                        ...
                        setImage(null);
                        setFloorValueStart("0");
                        setFloorValueEnd("0");
                        setShelvingValueStart("0");
                        setShelvingValueEnd("0");
                      }
                    });
                  }
                });
          ...
          <View style={{ width: "50%" }}>
            ...
            >
                ...
                <View style={{ flexDirection: "row" }}>
                  <View style={{ flex: 1 }}>
                    <FormInput
                      ...
                      value={shelvingValuestart}
                      onChange={(value) =>
                        setShelvingValueStart(
                          value === "" || value === "-"
                            ? (value = "0")
                            : NumberFormat(value)
                        )
              ...
              <View style={{ marginTop: 10 }}>
                <Text style={{ fontSize: 20, marginBottom: -10 }}>Floor</Text>
                <View style={{ flexDirection: "row" }}>
                  <View style={{ flex: 1 }}>
                    <FormInput
                      keyboardType="numeric"
                      value={floorValuestart}
                      onChange={(value) =>
                        setFloorValueStart(
                          value === "" || value === "-"
                            ? (value = "0")
