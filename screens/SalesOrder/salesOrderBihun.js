...
const SalesOrderBihun = ({ navigation }) => {
  ...
  useEffect(() => {
    // console.log("lokasi rmp", lokasi);
    // console.log("additional rmp", additional);
  ...
  const hitungulangtodos = async () => {
    ...
    if (hitung > 0) {
      ...
      await AsyncStorage.removeItem("salesrmp");
  ...
  const getTodosFromUserDevice = async () => {
    try {
      const todo = await AsyncStorage.getItem("salesrmp");
      if (todo != null) {
  ...
  const saveTodoToUserDevice = async (todos) => {
    ...
    try {
      ...
      await AsyncStorage.setItem("salesrmp", stringifyTodos);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  async function getOngkosAngkut(data) {
    // console.log("additionalrmp", data);
  ...
  const getLokasi = async () => {
    ...
    await GetDataLocal("additionalrmp").then(async (res1) => {
        ...
        axios
          ...
          .then(
            axios.spread(function (ongkosAngkut, listPrice, additionalCost) {
              // console.log("oa", ongkosAngkut.data);
              // console.log("pricelist", listPrice);
              // console.log(
              //   "additional cost",
      ...
      [
        {
          ...
          onPress: async () => {
            ...
            // console.log("data detail", dataSoDetail);
...
export default SalesOrderBihun;
