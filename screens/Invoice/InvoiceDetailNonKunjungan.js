...
const InvoiceDetailNonKunjungan = ({ navigation, route }) => {
  ...
  useEffect(() => {
    setPembayaran({ id: 3, label: "BANK TRANSFER", value: "BANK TRANSFER" });
  }, []);
  ...
  const simpanPembayaran = () => {
    if (pembayaran.id > 1 && image == null) {
      Alert.alert("Informasi", "Anda belum Melampirkan bukti bayar");
      return;
    }
    ...
    if (pembayaran.id >= 2 && alasanPembayaran === "") {
      Alert.alert("Warning", "Isi Nomor bukti transfer terlebih dahulu");
      return;
    }
    if (pilihNomorBpp === "") {
  ...
  function renderHeader() {
    return (
      <Header
        title="Detail Invoice Non Kunjungan"
        containerStyle={{
          height: 50,
      ...
      >
        ...
        <View>
          <TextInput
            ...
            selectTextOnFocus
            placeholder="Isi Nomor bukti transfer"
...
export default InvoiceDetailNonKunjungan;
